classCode=function(value,indicator="fertility"){
  if(indicator=="salinity"){
    #Salinity classes: 1-None; 2-Slight; 7-Very Slight; 3-Moderate; 4-Strong; 5-Very Strong; 6 - Extreme; 8 - Sodic; 9-Saline_sodic; 10-Salty
    classcode=ifelse(value<2,"None",ifelse(value<3,"Slight",ifelse(value<8,"VerySlight",
              ifelse(value<4,"Moderate",ifelse(value<5,"Strong",ifelse(value<6,"VeryStrong",
              ifelse(value<7,"Extreme",ifelse(value<10,"Saline-sodic",ifelse(value<11,"Alkaline","Class not included")))))))))
     }
